...
class ProfileController extends Controller
{
    public function profile($user_id)
    {
        ...
        $positions = profilesPositions::where('user_id', $user_id)
            ->select('position_name')
            ->join('positions', 'positions.position_id', '=', 'profilesPositions.position_id')
            ->get();

        $pro = Profiles::where('user_id', Auth::user()->user_id)->first();

        $pos = profilesPositions::where('user_id', Auth::user()->user_id)
            ->select('position_name')
            ->join('positions', 'positions.position_id', '=', 'profilesPositions.position_id')
            ->get();
    ...
    public function editProfile($user_id)
    {
        ...
        $user_positions = \DB::table('profilesPositions')
            ->where('user_id', $user_id)
            ->select('profilesPositions.position_id')
            ->join('positions', 'positions.position_id', '=', 'profilesPositions.position_id')
            ->get();
    ...
    public function updateProfile(Request $request, $user_id)
    {
        ...
        $pps = profilesPositions::where('user_id', $user_id)->get();
        foreach ($pps as $pp) {
            $arr1 .= $pp['position_id'] . " ";
        }
        if ($arr != $arr1) {
            profilesPositions::where('user_id', $user_id)->delete();
            foreach ($positions as $position) {
                $new = new profilesPositions();
                $new->user_id = $user_id;
        ...
        $ppsAfter = profilesPositions::where('user_id', $user_id)->get();
    ...
    {
        $position = new profilesPositions();
        $position->user_id = 10;
